...
class GeneralController extends Controller {
    ...
    public function scheduleRepairs(Request $request){
        ...
        if($user->can('general.repair')){
            ...
            if($keyword != '') $equipments= $equipments->where('equipments.title','like','%'.$keyword.'%');
            if($departments_id != '')$equipments= $equipments->whereHas('equipment_department', $equipments_query);
            $equipments = $equipments->withCount('schedule_repairs')->whereHas('schedule_repairs', $repair_query)->paginate($number);
            $total= $equipments->total();
            $data=[
                'keyword'=>$keyword,
                'departments_id'=>$departments_id,
    ...
    public function exportScheduleRepairs(Request $request) {
        ...
        $endDate = isset($request->endDate) ? date_format(date_create($request->endDate), 'Y-m-d') : Carbon::now()->format('Y-m-d');
        $key = isset($request->key) ? $request->key : '';
        return Excel::download(new ExportRepairRequestList($departments_id,$startDate,$endDate,$key), 'Báo cáo bảng kê yêu cầu sửa chữa ' . Carbon::now()->format('d-m-Y') . '.xlsx');
    }
    public function Liquidations(Request $request) {
        ...
        if($user->can('general.liquidation')){
            ...
            if($keyword != '') $liquidations= $liquidations->whereHas('equipment', $eq_query);
            ...
            $liquidations = $liquidations->orderBy('status','desc')->paginate($number);
    ...
    public function exportLiquidations(Request $request) {
        ...
        $key = isset($request->key) ? $request->key : '';
        $status_key = isset($request->status_key) ? $request->status_key : '';
        return Excel::download(new ExportLiquidationList($departments_id,$startDate,$endDate,$key, $status_key), 'Báo cáo bảng kê thanh lý thiết bị' . Carbon::now()->format('d-m-Y') . '.xlsx');
    }
    ...
    public function maintenanceEquipment(Request $request){
        ...
        if($user->can('general.maintenance')){
            ...
            if($keyword != '') $maintenances= $maintenances->whereHas('equipment', $eq_query);
            ...
            $maintenances = $maintenances->orderBy('maintenances.status','desc')->paginate($number);
