...
@section('title',__('Thêm Nhóm Thiết Bị'))
@section('content')
    ...
    <section class="content">
        <div class="container">
            <div class="head">
                ...
                <h1 class="title">{{ __('Thêm Nhóm Thiết Bị') }}</h1>
            </div>
            <div class="main">
                @include('notices.index')
                <form action="{{ route('equipment_cate.post') }}" class="dev-form" method="POST" data-toggle="validator" role="form">
                    @csrf
                    <div class="row">
                        <div class="col-md-9">
                            <div class="form-group">
                                <label class="control-label">{{ __('Tên nhóm thiết bị') }} <small>({{ __('require') }})</small></label>
                                <input type="text" name="title" value="{{ Request::old('title') }}" class="form-control" data-error="{{ __('Vui lòng nhập tên nhóm thiết bị')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Mã nhóm thiết bị') }} <small>({{ __('require') }})</small></label>
                                <input type="text" name="code" value="{{ Request::old('code') }}" class="form-control" data-error="{{ __('Vui lòng nhập mã nhóm thiết bị')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
