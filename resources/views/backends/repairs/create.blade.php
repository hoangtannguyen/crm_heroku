...
@section('title',__('Thêm đơn vị sửa chữa'))
@section('content')
@php 
    ...
    <section class="content">
        <div class="container">
            <div class="head">
                ...
                <h1 class="title">{{ __('Thêm đơn vị sửa chữa') }}</h1>
            </div>
            <div class="main">
                @include('notices.index')
                <form action="{{ route('repair.post') }}" class="dev-form" method="POST" data-toggle="validator" role="form">
                    @csrf
                    <div class="row">
                        <div class="col-md-9">
                            ...
                            <div class="form-group">
                                ...
                                <input name="note" type="text" class="form-control" value="{{ Request::old('note') }}" >
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Người liên hệ') }} <small>({{ __('require') }})</small></label>
                                <input name="contact" type="text" class="form-control" value="{{ Request::old('contact') }}" data-error="{{ __('Vui lòng nhập người liên hệ')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Số điện thoại') }} <small>({{ __('require') }})</small></label>
                                <input type="text" name="phone" value="{{ Request::old('phone') }}" class="form-control" data-error="{{ __('Vui lòng nhập số điện thoại')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Email') }} <small>({{ __('require') }})</small></label>
                                <input name="email" type="email" class="form-control" value="{{ Request::old('email') }}" data-error="{{ __('Vui lòng nhập email')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Địa chỉ') }} <small>({{ __('require') }})</small></label>
                                <input name="address" type="text" class="form-control" value="{{ Request::old('address') }}" data-error="{{ __('Vui lònge nhập địa chỉ')}}" requird>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
