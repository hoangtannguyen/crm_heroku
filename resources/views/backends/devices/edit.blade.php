...
@section('title',__('Sửa Loại thiết bị'))
@section('content')
    ...
    <section class="content">
        <div class="container">
            <div class="head">
                ...
                <h1 class="title">{{ __('Sửa Loại thiết bị') }}</h1>
            </div>
            <div class="main">
                @include('notices.index')
                <form action="{{ route('type_device.put' , $type_devices->id)}}" class="dev-form" method="POST" data-toggle="validator" role="form">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-9">
                            <div class="form-group">
                                ...
                                <input type="text" name="title" value="{{ $type_devices->title }}" class="form-control" data-error="{{ __('Vui lòng nhập tên loại thiết bị')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Mã loại thiết bị') }} <small>({{ __('require') }})</small></label>
                                <input type="text" name="code" value="{{ $type_devices->code }}" class="form-control" data-error="{{ __('Vui lòng nhập mã loại thiết bị')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                            <label class="control-label">{{ __('Nhóm thiết bị') }} <small>({{ __('require') }})</small></label>
                                ...
                                <option value="">Chọn nhóm thiết bị</option>
                                    @foreach ($equipment_cates as $equipment_cate)
                                        <option  {{ $equipment_cate->id == $type_devices->cat_id ? 'selected':'' }} value="{{ $equipment_cate->id }}">{{ $equipment_cate->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                                    ...
                                    <div class="card-body">
                                        <div id="frm-avatar" class="img-upload">
                                            <div class="image">
                                                ...
                                                {!! image($type_devices->image,230,230,__('Avatar')) !!}
                                                <input type="hidden" name="image" class="thumb-media" value="{{ $type_devices->image }}" />
                                            </div>
                                        </div>
                                    </div>
                            </aside>
