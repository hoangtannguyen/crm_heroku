...
@section('title',__('Sửa Loại Vật Tư'))
@section('content')
    ...
    <section class="content">
        <div class="container">
            <div class="head">
                ...
                <h1 class="title">{{ __('Sửa Loại Vật Tư') }}</h1>
            </div>
            <div class="main">
                @include('notices.index')
                <form action="{{ route('supplie.put' , $supplies->id)}}" class="dev-form" method="POST" data-toggle="validator" role="form">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-9">
                            <div class="form-group">
                                ...
                                <input type="text" name="title" value="{{ $supplies->title }}" class="form-control" data-error="{{ __('Vui lòng nhập tiêu đề hiển thị')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                            <div class="form-group">
                                <label class="control-label">{{ __('Mã vật tư') }} <small>({{ __('require') }})</small></label>
                                <input type="text" name="code" value="{{ $supplies->code }}" class="form-control" data-error="{{ __('Vui lòng nhập mã vật tư')}}" required>
                                <div class="help-block with-errors"></div>
                            </div>
                        </div>
                        <div class="col-md-3">
                                ...
                                <div class="card-body">
                                    <div id="frm-avatar" class="img-upload">
                                        <div class="image">
                                            ...
                                            {!! image($supplies->image,230,230,__('Avatar')) !!}
                                            <input type="hidden" name="image" class="thumb-media" value="{{ $supplies->image }}" />
                                        </div>
                                    </div>
                                </div>
                            </aside>
