            ...
            <div class="card-body p-0">
                  ...
                  <table class="table table-striped table-bordered" role="table">
                     <thead class="thead">
                        <tr class="text-center">
                           ...
                           <th>{{ __('Tên TB') }}</th>
                           <th>{{ __('Model') }}</th>
                           <th>{{ __('Tên họat động BD') }}</th>
                           <th>{{ __('Tần suất thực hiện') }}</th>
                     ...
                     <tbody class="tbody">
                        @if(!$maintenances->isEmpty())
                           @foreach($maintenances as $key => $main)
                              <tr class="text-center">
                                 <td>{{ ++$key}}</td>
                                 <td>{{ isset($main->equipment->equipment_department) ? $main->equipment->equipment_department->title : '-' }}</td>
                                 <td>{{ isset($main->equipment) && $main->equipment->code != null ? $main->equipment->code : '-' }}</td>
                                 <td>{{ isset($main->equipment) ? $main->equipment->title : '-' }}</td>
                                 <td>{{ isset($main->equipment) ? $main->equipment->model : '-'}}</td>
                                 <td><a href="{{ route('equip_maintenance.history',['equip_id'=>$main->equipment->id]) }}">{{ $main->title != null ? $main->title : '-' }}</a></td>
                                 <td>{{ $frequency[$main->frequency] }}</td>
                                 <td>{{ $main->start_date != null ? $main->start_date : '-' }}</td>
                                 <td>{{ $main->note != null ? $main->note : '-' }}</td>
                                 <td>{{ isset($main->author) ? $main->author->name : '-' }}</td>
                              </tr>
                           @endforeach  
                        @else
                        <tr>
               ...
               <form action="{{ route('general.maintenanceEquipment') }}" class="equipments" name="main_equipments" method="GET">
                  <div class="flex-load-page">
                     ...
                     {{ $maintenances->appends($data)->links() }}
                  </div>
               </form>
            </div>
         </div>
