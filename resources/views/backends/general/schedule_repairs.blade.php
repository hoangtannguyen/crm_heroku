      ...
      <div class="main">
         <div class="row search-filter">
            <div class="col-md-2 filter">
               <ul class="nav-filter">
                  <li class=""><a class="btn btn-success" style="color: #fff;" href="{{ route('general.exportScheduleRepairs',['department_id'=>$department_id,'startDate'=>$startDate,'endDate'=>$endDate,'key'=>$keyword]) }}"><i class="far fa-file-excel"></i> {{ _('Xuất Excel') }}</a></li>
               </ul>
            </div>
                     ...
                     <tbody class="tbody">
                        @if(!$repairs->isEmpty())
                           @foreach($repairs as $key => $repair)
                                 <tr class="text-center">
                                    <td>{{ ++$key}}</td>
                                    <td>{{ isset($repair->equipment->equipment_department) ? $repair->equipment->equipment_department->title : '-' }}</td>
                                    <td><a href="{{ route('eqrepair.history',['equip_id'=>$repair->equipment->id]) }}">{{ isset($repair->equipment) ? $repair->equipment->code : '-' }}</a></td>
                                    <td><a href="{{ route('eqrepair.history',['equip_id'=>$repair->equipment->id]) }}">{{ isset($repair->equipment) ? $repair->equipment->title : '-' }}</a></td>
                                    <td>{{ isset($repair->equipment->equipment_unit) ? $repair->equipment->equipment_unit->title : '-' }}</td>
                                    <td>{{ isset($repair->equipment) ? $repair->equipment->model : '-' }}</td>
                                    <td>{{ isset($repair->equipment) ? $repair->equipment->serial : '-' }}</td>
                                    <td>{{ $repair->planning_date }}</td>
                                    <td>{{ $repair->date_failure != '' ? $repair->date_failure : '-' }}</td>
                                    <td>{{ $repair->reason != '' ? $repair->reason : '-' }}</td>
                                    <td>{{ $status_acce[$repair->acceptance] }}</td>
                                 </tr>
                           @endforeach
