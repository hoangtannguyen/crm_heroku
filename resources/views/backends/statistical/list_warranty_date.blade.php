...
@section('title', __('Thống kê theo hết hạn bảo hành'))
@section('content')
<div id="list-departments" class="content-wrapper departments">
   <section class="content">
      <div class="head container">
         <h1 class="title">{{ __('Thống kê theo hết hạn bảo hành') }}</h1>
      </div>
      <div class="main">
         <div class="row search-filter">
            <div class="col-md-4 filter">
               <ul class="nav-filter">
                  <li class=""><a class="btn btn-success" style="color: #fff;" href="{{ route('statistical.exportWarrantyDate', ['startDate'=>$startDate , 'endDate'=>$endDate, 'key'=>$keyword]) }}"><i class="far fa-file-excel"></i> {{ _('Xuất Excel') }}</a></li>
               </ul>
            </div>
         ...
         <div class="card">
            <div class="card-body p-0">
               ...
               <form class="dev-form" action="{{ route('statistical.warrantyDate') }}" name="equipment_department" method="GET">
                  @csrf
                        ...
                        <tbody class="tbody">
                           @if(!$equipments->isEmpty())
                              ...
                              @foreach($equipments as $key => $equipment)
                                    ...
                                    <tr class="text-center">
                                       <td>{{ ++$key}}</td>
                                       <td>{{ date('d-m-Y', strtotime($equipment->warranty_date))}}</td>
               ...
               <form action="{{ route('statistical.warrantyDate') }}" class="equipments" name="equipments_department" method="GET">
                  ...
                  <div class="flex-load-page">
                     ...
                     @if($startDate=="" && $endDate=="" && $keyword=="")
                        {!! $equipments->links() !!}
                     @else
                        {!! $equipments->appends(['startDate'=>$startDate, 'endDate'=>$endDate, 'key'=>$keyword])->links() !!}     
                     @endif
                  </div>
               </form>
            </div>
